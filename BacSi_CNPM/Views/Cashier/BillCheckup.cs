...
namespace Hospital.Views.Cashier
{
    public partial class BillCheckup : Form
    {
        ...
        public BillCheckup()
        {
            ...
            InitializeComponent();
            LoadBenhNhan();
            this.dtgv_patientbills.Visible = false;
        }
        ...
        private void dtgv_patient_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.RowIndex < dtgv_patient.Rows.Count)
            {
                ...
                if (cellValue != null)
                {
                    this.dtgv_patientbills.Visible = true;
        ...
        private void dtgv_patientbills_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.RowIndex < dtgv_patientbills.Rows.Count)
            {
                ...
                if (cellValue != null)
                {
                    ...
                    if (cashier.CheckHD(HD_ID))
                    {
                        showMiddleForm(new BillDetail(HD_ID, tongtien, thanhtoan));
                    }
                    else
                    {
                        showMiddleForm(new ExamineDetail(HD_ID, tongtien, thanhtoan));
                    }    
        ...
        {
            if (tb_searchpatient.Text != "")
            {
                dtgv_patient.DataSource = cashier.SearchPatient(tb_searchpatient.Text);
            }
            else
            {
            }
